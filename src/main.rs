    ...
    let eq = loop {
        ...
        match &*option.trim() {
            "1" => {
                ...
                println!("Chosen: Linear Equation");
                break EquationType::Linear;
            }
            "2" => {
                clearscreen::clear().expect("failed to clear screen");
                println!("Chosen: Quadratic Equation");
                break EquationType::Quadratic;
            }
            "3" => {
                clearscreen::clear().expect("failed to clear screen");
                println!("Chosen: Cubic Equation");
                break EquationType::Cubic;
            }
            "4" => {
                clearscreen::clear().expect("failed to clear screen");
                println!("Chosen: Quartic Equation");
                break EquationType::Quartic;
            }
            _ => {
                println!("{option} is not an option!");
                continue;
            }
        };
    };
